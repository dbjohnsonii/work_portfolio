...
(SELECT paymentprocessornumber,
        CASE WHEN SERVICEACCOUNTNAME ='CommWeb' then 'Commonwealth'
        WHEN SERVICEACCOUNTNAME ='MerchantSolutions' then 'Merchant Solutions'
        WHEN SERVICEACCOUNTNAME ='Wells Fargo MC' then 'Wells Fargo'
        WHEN SERVICEACCOUNTNAME LIKE '%Litle%' then 'Vantiv'
        WHEN SERVICEACCOUNTNAME LIKE '%Vantiv%' then 'Vantiv'
        WHEN SERVICEACCOUNTNAME LIKE '%First Data%' then 'First Data'
        WHEN SERVICEACCOUNTNAME LIKE '%Firstdata%' then 'First Data'
...
(SELECT 
    ...
    WHERE MATCH_DATE BETWEEN TO_DATE('04/01/2018','MM/DD/YYYY') AND TO_DATE('04/30/2018','MM/DD/YYYY')
...
AND co.IIN=cc.BIN
WHERE pa.statusdate BETWEEN TO_DATE('04/01/2018','MM/DD/YYYY') AND TO_DATE('04/30/2018','MM/DD/YYYY')
AND pa.statusid in (525,600)
and paymentproductid=3
and pp.interchange_region NOT IN('Europe','EMEA')
